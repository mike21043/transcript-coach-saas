...
Cleanup (local/dev only)
- During routine maintenance we remove temporary test artifacts and local editor logs that are not required for the project itself. Common files removed on the development host include:
  - test audio and quick inspection scripts (for example `test.wav`, `tmp_inspect_raw.py`, `tmp_dump_offers.py`)
  - temporary media stash under `tmp-stash/`
  - build and npm debug logs (for example `build.log`, files under `.npm/_logs/`)
  - editor/server logs under `.vscode-server/data/logs`

These are developer convenience files and are safe to recreate if needed. They are not required to build or run the project and are intentionally excluded from source control.

Note about debug data
- Large or detailed debug artifacts collected during runs (for example logs under `debug/` and captured `instance_consoles/`) have been moved to `debug/archived/` to keep the repo working tree clean while preserving them for investigation. To permanently remove archived debug data run:

  rm -rf debug/archived/

  To restore individual files, move them back out of `debug/archived/`.
